using iCUE_ReverseEngineer.Icue.Handlers;
...
public sealed class GameHandler : IDisposable
{
    ...
    public GameHandler(IcueToGameConnection gameConnection)
    {
        ...
        _sdkHandler = new SdkHandler(_gameConnection);
    ...
    private void OnGameMessageReceived(object? sender, IcueGameMessage message)
    {
        ...
        if (!_gsiHandler.GameHandles.TryGetValue(messageMethod, out var handle) && 
            !_sdkHandler.SdkHandles.TryGetValue(messageMethod, out handle))
        {
